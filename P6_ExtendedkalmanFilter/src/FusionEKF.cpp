...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    //cout<<"init params in KF finish"<<endl;

    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      /**
      Convert radar from polar to cartesian coordinates and initialize state.
      */
      float rho = measurement_pack.raw_measurements_(0);  // rho
      float theta = check_theta(measurement_pack.raw_measurements_(1));  // theta
      float rho_p = measurement_pack.raw_measurements_(2);  // rho_p
      ekf_.x_(0) = rho * cos(theta);  // px
      ekf_.x_(1) = rho * sin(theta);  // py
      //ekf_.x_(2) = rho_p * cos(theta);  // vx
      //ekf_.x_(3) = rho_p * sin(theta);  // vy
  ...
  if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
    // Radar updates
    float theta = check_theta(measurement_pack.raw_measurements_(1));  
    // cout<<"theta: "<<theta<<" theta meas: "<<measurement_pack.raw_measurements_(1)<<endl;
