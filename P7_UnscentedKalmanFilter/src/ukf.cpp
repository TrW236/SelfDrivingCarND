...
UKF::UKF() {
  ...
  Xsig_pred_ = MatrixXd(n_x_, 2*n_aug_+1);  //  15 points, the dimension 5 x 15
...
void UKF::Prediction(double delta_t) {
  ...
  MatrixXd Xsig_aug = MatrixXd(n_aug_, 2 * n_aug_ + 1);  // colume is each sigma point
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  for (int i = 0; i < 2*n_aug_+1; ++i)  // loop through every sigma point
  {
